...
- [ x ] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  -Client side routing takes place wherever the application has loaded, or at the client. All routes are defined statically within the App's structure and no server requests are made unless explicitly defined in the applications architecture.

  -Server side routing takes place on a central server for all clients that may connect to the application. For every request for data from the client, the server will receive the request and respond with the requested data.  
...
- [ x ] Mention three tools we can use to make AJAX requests.
  ...
  -Fetch, Then and Catch methods are all tools used to make AJAX requests.
